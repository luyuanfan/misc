...
// Jasmine: mark
// *** in lib/sbi/messages.h ===> header type
...
// *** lib/sbi/messages.h ====> request type
...
// Jasmine: so service name is not it. 

// *** lib/sbi/message.h ====> message type
...
// Jasmine: maybe we can change the NF status
// /08 19:50:38.275: [nrf] INFO: [1c7a5f78-eccb-41f0-a410-51e276ce3bea:0]
// NF-Discovered [NF-Type:UDM,NF-Status:REGISTERED,IPv4:1,IPv6:0,FQDN:udm.5gc.mnc070.mcc999.3gppnetwork.org]
// (../src/nrf/nnrf-handler.c:1087)

// void change_sn_id (ogs_sbi_request_t *request) {

//     char* old_uri = request->h.uri;
//     char* new_uri = NULL;
//     char* target = "mnc001.mcc001";
//     char* rep = "mnc310.mcc451"; // arbitrary network sn id
//     char* start_ptr = strdup(old_uri);
//     char* mid_ptr = strstr(start_ptr, target);
//     char* end_ptr = NULL;
//     if ( mid_ptr == NULL) {
//         return;
//     }
//     end_ptr = mid_ptr + strlen(start_ptr);
//     printf("@@@start_ptr: %s", start_ptr);
//     printf("@@@mid_ptr: %s", mid_ptr);
//     printf("@@@end_ptr: %s", end_ptr);
//     mid_ptr = strcat(rep, end_ptr);
//     new_uri = strcat(start_ptr, mid_ptr);
//     printf("@@@new_uri: %s", new_uri);
//     // request->h.uri = new_uri;
// }
        ...
        void *data)
{
    ...
    // change_sn_id(request);
    // printf("@service in reheader: %s", request->h.service.name);
    // printf("@method in reqheader: %s", request->h.method);
    // printf("@uri in reqheader: %s", request->h.uri);
    ...
    // allocate a connection from the connection pool
    ...
    // assign the requesting client & client data to this new connection 
    conn->client = client;
    ...
    // use curl easy library to handle the given request
